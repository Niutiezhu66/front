    ...
    <div class="user-manage-container">
          ...
          <el-card class="box-card">
            ...
            <div style="text-align: center; padding: 30px 0;">
              <el-button type="primary" size="large" @click="showBatchImportDialog">
                <el-icon><UploadFilled /></el-icon> 批量导入师生 (Excel)
              </el-button>
            </div>
          </el-card>
        </el-col>
      ...
      >
        ...
        <div v-show="batchImportStep === 0" class="step-content">
          <div class="template-download">
            <el-button type="primary" link @click="downloadTemplate">
              <el-icon><Download /></el-icon> 点击下载Excel导入模板
            </el-button>
          </div>
        ...
        <div v-show="batchImportStep === 1" class="step-content">
          ...
          <el-table :data="previewData" height="300" border stripe>
            <el-table-column prop="username" label="系统账号" width="120" />
            <el-table-column prop="password" label="密码" width="100" />
            <el-table-column prop="realName" label="真实姓名" width="100" />
            <el-table-column prop="roleStr" label="角色(1老师/2学生)" width="150" />
            <el-table-column prop="userIdStr" label="学号/工号" />
          </el-table>
...
const downloadTemplate = async () => {
  try {
    // 1. 使用项目中统一封装的 request，确保自动携带 Token
    // 2. 必须设置 responseType: 'blob'，否则下载的文件内容会乱码/损坏
    const response = await request({
      url: '/api/user/batch/template',
      method: 'GET',
      responseType: 'blob' 
    })

    // 根据你的 axios 拦截器配置，这里可能是 response.data 也可能是直接返回的 response
    const fileData = response.data ? response.data : response
    
    // 3. 显式指定正确的 Excel MIME 类型
    ...
    ElMessage.error('模板下载失败，请检查网络或联系管理员')
  }
}
  ...
  .relation-form { margin-top: 20px; }
  .step-content { margin-top: 30px; text-align: center; }
  .template-download { text-align: right; margin-bottom: 10px; }
